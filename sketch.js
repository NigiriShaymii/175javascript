...
var time = 45; // Length of recording in 45 seconds 
  ...
  if(seconds == 25)
    {
      ...
      reverb.process(cough, 3, 3);
      reverb.process(chant, 3, 3);
      reverb.process(burn, 3, 3);
      reverb.process(vote, 3, 3);
    }
  if(seconds == time)
        phase = 3;
...
function reveal() {
  ...
  fill(255);
  text("Your voice is a part of a data collection of outbursts, confessions, and hope. \nThe message you spoke will not be remembered virtually but cherished biologically.\n\n Our stolen time became manipulated noises. \nIt echoes into our history as a fading pixel in an infinity plane on a monitor", width/2, height/2);
}
...
function timer() {
  ...
  if(seconds === time) // NOTE TO SELF: SET 59 AFTER EVERYTHING WORKS
  {
    // Stops replay
    record.stop();
